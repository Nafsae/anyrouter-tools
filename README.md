...
- **API Key 管理** — 每个账号可存储 API Key（令牌），支持内联编辑、复制、一键测试是否可用
- **批量测试 API Key** — 一键并发测试所有已设置的 API Key 可用性
- **一键导出全部 Cookie** — 所有账号 Cookie 导出为 JSON，复制到剪贴板或保存为文件
...
- **Keychain 安全存储** — 所有 Cookie 和 API Key 统一存储在 Keychain，只需授权一次
- **macOS 通知** — 签到结果推送通知
...
│   ├── KeychainService.swift            # Keychain 统一存储 Cookie 和 API Key
│   ├── NotificationService.swift        # macOS 通知
│   └── SchedulerService.swift           # 定时刷新 + 每日自动签到调度
├── ViewModels/
│   ├── AccountListViewModel.swift       # 账号列表 + 刷新/签到/导出/Key测试
...
| UI 框架 | SwiftUI |
| 数据持久化 | SwiftData |
| Cookie/Key 存储 | macOS Keychain（统一 JSON 存储） |
